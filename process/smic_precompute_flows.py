...
def construct_image_path(row, raf_path):
    ...
    onset = str(row['OnsetFrame']).zfill(6)
    apex = str(row['ApexFrame']).zfill(6)
    offset = str(row['OffsetFrame']).zfill(6)
    ...
    # 构造路径
    path_on = os.path.join(raf_path, 'SMIC','HS',subject,'micro',file_label,filename,on0_j)
    path_apex = os.path.join(raf_path, 'SMIC','HS',subject,'micro',file_label,filename,apex0_j)
...
def generate_flow_filename(row):
    ...
    # 生成唯一文件名
    flow_filename = f"flow_smic_{subject}_{clean_filename}_{onset}_{apex}.npy"
...
if __name__ == '__main__':
    ...
    # 1. 原始CSV文件路径
    CSV_PATH = '/nfs/users/yanghuiru/AMER/dataset/SMIC/smic_coding.csv'
    
    # 2. 数据集根路径
    RAF_PATH = '/nfs/users/yanghuiru/AMER/dataset'
    
    # 3. 输出目录
    OUTPUT_DIR = '/nfs/users/yanghuiru/AMER/dataset/SMIC/precomputed_flows'
