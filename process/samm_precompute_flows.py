...
def construct_image_path(row, raf_path):
    ...
    # 构造文件名
    on0_j =f"{subject}_{onset}.jpg"
    apex0_j = f"{subject}_{apex}.jpg"
...
def generate_flow_filename(row):
    ...
    # 生成唯一文件名
    flow_filename = f"flow_samm_{subject}_{clean_filename}_{onset}_{apex}.npy"
...
def precompute_flows(csv_path, raf_path, output_dir, overwrite=False):
    ...
    if file_ext == '.csv':
        ...
        try:
            # 尝试读取所有工作表，或者指定工作表
            df = pd.read_excel(csv_path,usecols=[0, 1, 3, 4, 5, 8, 9])
            df.columns = ['Subject', 'Filename', 'OnsetFrame', 'ApexFrame', 'OffsetFrame', 'Label_AU', 'Label']
...
if __name__ == '__main__':
    ...
    # 1. 原始CSV文件路径
    CSV_PATH = '/nfs/users/yanghuiru/AMER/dataset/SAMM/SAMM_Micro_FACS_Codes_v2.xlsx'
    
    # 2. 数据集根路径
    RAF_PATH = '/nfs/users/yanghuiru/AMER/dataset'
    
    # 3. 输出目录
    OUTPUT_DIR = '/nfs/users/yanghuiru/AMER/dataset/SAMM/precomputed_flows'
