...
def construct_image_path_3db(row, raf_path):
    """
    为3DB数据集构造图像路径
    ...
    if dataset == 'casme2':
        ...
        path_on = os.path.join(raf_path, 'CASMEII', 'Cropped', 
                              subject, filename, on0_j)
        path_apex = os.path.join(raf_path, 'CASMEII', 'Cropped', 
                                subject, filename, apex0_j)
        
    elif dataset == 'samm':
        # SAMM数据集
        # 处理subject格式，确保是3位数字
        sub_num = str(subject).zfill(3)
        
        # 构造文件名
        on0_j = f"{sub_num}_{onset}.jpg"
        apex0_j = f"{sub_num}_{apex}.jpg"
        
        # 构造路径
        path_on = os.path.join(raf_path, 'SAMM', 'Cropped', 
                              sub_num, filename, on0_j)
        path_apex = os.path.join(raf_path, 'SAMM', 'Cropped', 
                                sub_num, filename, apex0_j)
        
    elif dataset == 'smic':
        # SMIC数据集
        # 处理subject格式
        if isinstance(subject, str) and subject.startswith('s'):
            sub_num = subject[1:]  # 从's01'中提取'01'
        else:
            sub_num = str(subject)
        ...
        # 构造文件名
        on0_j = 'reg_image' + onset + '.bmp'
        apex0_j = 'reg_image' + apex + '.bmp'
        
        # 处理subject格式
        sub = 's' + str(int(sub_num))
        
        # 处理filename和标签
        if isinstance(filename, str):
            if 'ne' in filename.lower():
                file_label = 'negative'
            elif 'po' in filename.lower():
                file_label = 'positive'
            elif 'su' in filename.lower():
                file_label = 'surprise'
            else:
                file_label = 'micro'  # 默认值
            
            # 调整filename格式
            f = sub + filename[3:] if len(filename) > 3 else filename
        else:
            file_label = 'micro'
            f = str(filename)
        
        # 构造路径
        path_on = os.path.join(raf_path, 'SMIC', 'HS', sub, 'micro', 
                              file_label, f, on0_j)
        path_apex = os.path.join(raf_path, 'SMIC', 'HS', sub, 'micro', 
                                file_label, f, apex0_j)
...
def generate_flow_filename_3db(row, flow_type='farneback'):
    """为3DB数据集生成光流文件名"""
    dataset = row['Dataset']
    subject = row['Subject']
    filename = row['Filename']
    ...
    if flow_type == 'farneback':
        flow_filename = f"flow_farneback_{dataset}_{subject}_{clean_filename}_{onset}_{apex}.npy"
    elif flow_type == 'tvl1':
        flow_filename = f"flow_tvl1_{dataset}_{subject}_{clean_filename}_{onset}_{apex}.npy"
    else:
        flow_filename = f"flow_{dataset}_{subject}_{clean_filename}_{onset}_{apex}.npy"
    
    return flow_filename


def precompute_dual_flows_3db(csv_path, raf_path, output_dir, overwrite=False):
    """
    为3DB数据集预计算双光流
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="构造路径"):
        try:
            path_on, path_apex = construct_image_path_3db(row, raf_path)
            image_paths.append({
                'path_on': path_on,
                'path_apex': path_apex
            })
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="计算光流"):
        ...
        # 生成两种光流文件名和路径
        farneback_filename = generate_flow_filename_3db(row, flow_type='farneback')
        tvl1_filename = generate_flow_filename_3db(row, flow_type='tvl1')
    ...
    for dataset in datasets:
        ...
        farneback_valid = dataset_df[dataset_df['FlowPath_Farneback'] != '']
        tvl1_valid = dataset_df[dataset_df['FlowPath_TVL1'] != '']
        both_valid = dataset_df[(dataset_df['FlowPath_Farneback'] != '') & 
                               (dataset_df['FlowPath_TVL1'] != '')]
        
        print(f"\n{dataset.upper()}数据集:")
        print(f"  总样本数: {len(dataset_df)}")
        print(f"  有效的Farneback光流: {len(farneback_valid)} ({len(farneback_valid)/len(dataset_df)*100:.1f}%)")
        print(f"  有效的TV-L1光流: {len(tvl1_valid)} ({len(tvl1_valid)/len(dataset_df)*100:.1f}%)")
        print(f"  两种光流都有效的: {len(both_valid)} ({len(both_valid)/len(dataset_df)*100:.1f}%)")
    ...
    if len(failed_both) > 0:
        ...
        for idx, row in failed_both.head(10).iterrows():
            ...
            print(f"    图像路径: {row['OnsetPath']} -> {row['ApexPath']}")
    
    return output_csv


def verify_dual_flows_3db(csv_path, sample_check=3):
    """验证3DB数据集双光流预计算结果"""
    print(f"\n验证3DB数据集双光流预计算结果...")
    df = pd.read_csv(csv_path)
    ...
    for idx in test_indices:
        ...
        print(f"\n样本{idx}: {row['Dataset']}/{row['Subject']}/{row['Filename']}")
...
def main():
    # ==================== 配置参数 ====================
    # 1. 原始CSV文件路径
    CSV_PATH = '/nfs/users/yanghuiru/AMER/dataset/combined_3class_with_frames.csv'
    
    # 2. 数据集根路径
    RAF_PATH = '/nfs/users/yanghuiru/AMER/dataset'
    
    # 3. 输出目录
    OUTPUT_DIR = '/nfs/users/yanghuiru/AMER/dataset/3DB/precomputed_dual_flows'
    
    # 4. 是否覆盖已存在的光流文件
    OVERWRITE = False   # True: 重新计算所有, False: 跳过已存在的
    ...
    print("\n开始计算两种光流...")
    new_csv = precompute_dual_flows_3db(CSV_PATH, RAF_PATH, OUTPUT_DIR, OVERWRITE)
    
    # 验证结果
    if VERIFY and new_csv and os.path.exists(new_csv):
        verify_dual_flows_3db(new_csv)
    else:
        print(f"警告: 无法验证结果，CSV文件不存在")
    
    print(f"\n{'='*60}")
    print("3DB数据集双光流预计算任务完成！")
    print(f"{'='*60}")
