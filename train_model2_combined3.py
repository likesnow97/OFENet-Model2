...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--csv-relpath", type=str, default="3DB/precomputed_tvl1_flows_224/dataset_with_tvl1_flows_224.csv")
...
def get_subjects(args: argparse.Namespace) -> List[str]:
    csv_path = Path(args.raf_path) / args.csv_relpath
    if not csv_path.exists():
        raise FileNotFoundError(f"CSV not found: {csv_path}")

    df = pd.read_csv(csv_path)
    if "Subject" not in df.columns:
        raise KeyError(f"`Subject` column not found in {csv_path}. Columns: {df.columns.tolist()}")

    all_subjects = df["Subject"].dropna().astype(str).unique().tolist()

    if args.subjects.strip().lower() == "all":
        subjects = all_subjects
    else:
        required = [s.strip() for s in args.subjects.split(",") if s.strip()]
        unknown = [s for s in required if s not in all_subjects]
        if unknown:
            raise ValueError(f"Unknown subject keys: {unknown}. Available examples: {all_subjects[:30]}")
        subjects = required
...
def main() -> None:
    ...
    print("=== strict_model2 train_model2_combined3 ===")
